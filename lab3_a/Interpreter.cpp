...
void Interpreter::perform_body()
{
	...
	*/
	...
	//Каждый парсер самостоятельно двигает GI, цикл завершается при достижении return функции main
	while (1)
	{
				...
				for (int i = 0; i < args.size(); i++)
				{
					...
					if (regex_match(args[i].c_str(), local_cg, var_operand)) //Переменная
					{
						...
						case vt_Integer:
							...
							break;
						default:
							throw (std::string)("This type of argument is not supported");
							break;
						}
			...
			case vt_String:
				...
				else if (cg[4].length() != 0) //Строка
				{
					...
					else //Выражение
					{
						...
						ST_Calculator str_calculator(crutch); //строится дерево
		...
		GI++; //Пропускает строку если интерпретатор никак ее не обработал 
